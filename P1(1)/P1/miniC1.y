...
program         :  {tablaSimb=creaLS();} ID LPAREN RPAREN LLLAVE declarations statement_list RLLAVE {printf("Aplica program -> ID () { declarations statement_list } \n");}
...
perteneceTablaS(Simbolo s) {
	p = buscaLS(tablaSimb, s.nombre);
 	if (p != finalLS(tablaSimb)) {
    		Simbolo aux = recuperaLS(tablaSimb,p);
    		printf("Encontrado %s, s.nombre, tipo: %s\n",(aux.tipo == VARIABLE ? "variable" : "constante"));
  	} else {
    		printf("No encontrado %s\n", s.nombre);
  	}
}

insertaTablaIdentificador(Simbolo s, Tipo t) {
	if(t == CONSTANTE) {
...
bool esConstante(Simbolo s) {
	return s.tipo == CONSTANTE;
}

insertaTablaString(Simbolo s, Tipo t, int contCadenas) {
	Simbolo aux;
	aux.nombre = strcat("str", to_string(contCadenas));
	aux.tipo = t;
	aux.valor = s.valor;
